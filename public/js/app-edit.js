const addColor = (event, size) => {
    ...
    $(`.colors-${size}`).append(`<div class="col-12 inputs_${lastIndex}_${size}">
        <input name="color_${lastIndex}_${size}" type="color" class="form-control inputStock form-control-color my-2"
    id="exampleColorInput" value="#563d7c" title="Choose your color">
        <input name="stock_${lastIndex}_${size}" type="number"
    class="form-control inputStock form-control-color" id="stock">
    <button onclick="removeColor(event, ${lastIndex}, '${size}')" class="btn btn-danger mt-2 inputStock btn-small"
...
$('#add-image').on('click', event => {
    ...
    $('.images-container').append(`<div class="row mx-auto col-4" id="image-container-${i}">
        ...
        <input type="color" name="image_color_${i}" class="form-control form-control-color inputStock" />
